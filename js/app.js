(function ($) {
    ...
    var options = {
        ...
        onAfterViewLoad: function (view) {
            ...
            $('.page-header .month-name').text(this.getTitle());
            $('.jqyc-year').text(this.getTitle());
            $('.btn-group button').removeClass('active');
            $('button[data-calendar-view="' + view + '"]').addClass('active');
            if (view == "month") {
                $('.btn.btn-default').css('display', 'block');
            }
            if (view == "year") {
    ...
    $('.btn-group button[data-calendar-nav]').each(function () {
        ...
        $this.click(function () {
            debugger
            if ($this.val() == "next-btn") {
                calendar.navigate($this.data('calendar-nav'));
                $('#next-year-btn').click();
            }
            else if ($this.val() == "prev-btn") {
                calendar.navigate($this.data('calendar-nav'));
                $('#prev-year-btn').click();
            }
            else {
...
(function ($) {
    ...
    $.fn.calendar = function (options) {
        ...
        if (methods[options]) {
            ...
            function showPreviousYear() {
                ...
                var currentYear = parseInt($('.month-name').text());
                settings.startYear = currentYear;
    ...
    function createWholeCalendar(settings, $this) {
        ...
        // var flagprev = 0;
        // if (flagprev.val() == 'prev') {
        //     var plusyear = $('.month-name').text();
        //     var orgyear = parseInt(plusyear, 10);
        //     var newyear = orgyear + 1;
        // }
        // var plusyear = $('.month-name').text();
        // var orgyear = parseInt(plusyear, 10);
        // var newyear = orgyear + 1;
        ...
        var domSkeleton = '<div class="jqyc" ><div class="jqyc-year-chooser border-top border-bottom row align-items-center" data-current-year="' + year + '">' +
            '<div class="' + colColXs2 + ' text-left" style="display:none;">' +
            '   <button type="button" id="prev-year-btn" class="btn btn-primary btn-sm jqyc-prev-year jqyc-change-year" data-year="' + (year - 1) + '">&larr;</button>' +
            '</div>' +
            '<div class="jqyc-year ' + colColXs4 + '"  style="display:none;">' + year + '</div>' +
            '<div class="' + colColXs2 + ' text-right"  style="display:none;">' +
            '   <button type="button" id="next-year-btn" class="btn btn-primary btn-sm jqyc-next-year jqyc-change-year" data-year="' + (year) + '">&rarr;</button>' +
            '</div>' +
            '</div>' +
        ...
        if (startFromSunday) {
            var tableSkeleton = '<table class="table table-sm jqyc-table">' +
                ' <div role="tooltip"  class="date-active-modal popover in popover-right bs-popover-right right bs3">'
                + '  <div class="popover-arrow arrow" style="top: 44px;"></div>'
                + ' <div class="popover-content popover-body">'
                + ' <div  class="col-md-12 col-calendar ng-star-inserted">'
                + ' <p  class="pop_year_day">We.</p>'
                + '<p  class="pop_year_day_number">18 </p>'
            ...
            var tableSkeleton = '<table class="table table-sm jqyc-table">' +
                ...
                + '<thead>' +
                '<tr class="jqyc-tr jqyc-thead-tr">' +
                '<th class="jqyc-th mon" scope="col">' + settings.l10n.mn + '</th>' +
                '<th class="jqyc-th tue" scope="col">' + settings.l10n.tu + '</th>' +
                '<th class="jqyc-th wed" scope="col">' + settings.l10n.we + '</th>' +
                '<th class="jqyc-th thu" scope="col">' + settings.l10n.th + '</th>' +
                '<th class="jqyc-th fri" scope="col">' + settings.l10n.fr + '</th>' +
                '<th class="jqyc-th sat" scope="col">' + settings.l10n.sa + '</th>' +
                '<th class="jqyc-th sun" scope="col">' + settings.l10n.su + '</th>' +
                '</tr>' +
                '</thead>' +
                '<tbody></tbody>' +
                '</table>';
        ...
        if (settings.showHeaders) {
            $html.find('.jqyc-jan').append('<h5 class="jqyc-header" id=' + settings.l10n.jan + '>' + settings.l10n.jan + '</h5>');
        }
    ...
    function jqycGetMonthHTMLStringWithData(firstDay, month, year, days = 31) {
        ...
        while (d <= days) {
            ...
            if (i < firstDay) {
                d--;
                monthHTMLString = monthHTMLString + '<td class="jqyc-empty-td jqyc-td" id="date-deactive-modal"></td>';
            } else {
                monthHTMLString = monthHTMLString +
                    '<td id="date-deactive-modal' + month + d + '" class="jqyc-not-empty-td jqyc-td jqyc-day-' + d
                    + ' jqyc-day-of-' + month + '-month" data-month="' + month
                    + '" data-day-of-month="' + d + '" data-year="' + year + '" onclick="jan(' + month + d + ',' + i + ',' + d + ')" style="position: relative;">' + d + ' </td>';
            }
            if (i % 7 == 0) {
